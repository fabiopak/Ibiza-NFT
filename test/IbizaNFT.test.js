...
//const IbizaNFT = contract.fromArtifact('IbizaNFT');

contract('IbizaNFT', function (accounts) {
  ...
  beforeEach(async function () {
    this.token = await IbizaNFT.new(name, symbol, baseURI, { from: deployer });
  });
  ...
  describe('minting', function () {
    ...
    it('deployer can mint tokens with tokenID', async function () {
      const tokenId = new BN('12345');

      const receipt = await this.token.mintTokenID(other, tokenId, { from: deployer });
      expectEvent(receipt, 'Transfer', { from: ZERO_ADDRESS, to: other, tokenId });
    ...
    it('deployer can mint tokens complete', async function () {
      const tokenId = new BN('12346');

      const receipt = await this.token.mintComplete(other, tokenId, "pippo", { from: deployer });
      expectEvent(receipt, 'Transfer', { from: ZERO_ADDRESS, to: other, tokenId });
    ...
    it('other accounts cannot mint tokens', async function () {
      await expectRevert(
        ...
        'IbizaNFT: must have minter role to mint',
      );
    });
  });
  ...
  describe('pausing', function () {
    ...
    it('other accounts cannot pause', async function () {
      await expectRevert(
        ...
        'IbizaNFT: must have pauser role to pause',
      );
    });
    ...
    it('other accounts cannot unpause', async function () {
      ...
      await expectRevert(
        ...
        'IbizaNFT: must have pauser role to unpause',
      );
    });
  });
